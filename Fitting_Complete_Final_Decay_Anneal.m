...
% Very similar to the more commented partial feedback codes 
clear all
close all

%% Selecting the dataset 

% load P2017b % Palminteri 2017 (complete feedback experiment)

 load S2021c;sta=stac;cho=choc;out=outc;cou=couc; % Katahira complete
...
% cou: counterfatual outcome (-1 or 1)

subjecttot=numel(sta); 
%%


n_model=3; % because we are fitting three models
...
for k_sub = 1:subjecttot
    ...
    for k_model = 1:n_model
        ...
        for k_rep = 1:n_rep
            ...
            [parametersLPP_rep(k_rep,1:nfpm(k_model)),LPP_rep(k_rep),~,~,~,~,FminHess(k_rep,:,:)]=fmincon(@(x) Priors_Complete_Final_Decay_Anneal(x,sta{k_sub},cho{k_sub},out{k_sub},cou{k_sub},k_model),x0,[],[],[],[],LB,UB,[],options);
        end
